...
Describes the Water Linked DVL serial protocol for DVL A100 and DVL A250. For PD0, PD4, and PD6 output formats, see [PD formats](dvl-pd-formats.md).
...
A velocity report is output for each velocity calculation of the DVL. The rate depends on the model, altitude, and configuration. See the model page and [Range mode](range-mode.md) for product-specific operating limits.
...
<!-- TODO: Engineering review: confirm RX-line trigger timing requirements and valid serial configuration assumptions. -->
